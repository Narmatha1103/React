...
const Remainder = () => {
  ...
  useEffect(() => {
    const getRemainders = async () => {
      const remainderSnapshot = await getDocs(collection(db, "product-app"));
  ...
  const submitRemainder = async (e) => {
    ...
    e.preventDefault();
    try {
      await addDoc(collection(db, "product-app"), {
  ...
  const deleteRemainder = async (id) => {
    try {
      if (window.confirm("Are you sure to delete this remainder?")) {
        const docRef = doc(db, "product-app", id);
  ...
  const handleChecked = async (e, remainder) => {
    ...
    const docRef = doc(db, "product-app", e.target.name);
    try {
